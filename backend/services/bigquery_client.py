...
class BigQueryClient:
    ...
    async def initialize(self) -> None:
        """Ensure dataset/tables exist (Central auth handled by config.py)."""
        if not self._client:
            # Try to re-init if client is None (maybe auth was set after __init__)
            try:
                from google.cloud import bigquery
                self._client = bigquery.Client(project=self._project_id)
            except Exception as e:
                logger.error("bq.init.retry_failed", error=str(e))
                return
